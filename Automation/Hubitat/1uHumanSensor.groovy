...
private Map parseIasMessage(String description) {
    ...
    def motionState = translateZoneStatus(zs)
    sendEvent(motionState)
    updatePresence(motionState) // Update presence based on motion state
    return motionState
}
...
private Map translateZoneStatus(ZoneStatus zs) {
    ...
    motionState.value = (zs.isAlarm1Set() || zs.isAlarm2Set()) ? 'active' : 'inactive'
    motionState.descriptionText = "$device.displayName motion is ${motionState.value}"
    return motionState
}

def updatePresence(Map motionState) {
    def presenceState = motionState.value == "active" ? "present" : "not present"
    sendEvent(name: "presence", value: presenceState)
    logDebug "Presence updated to $presenceState"
}
